...
SELECT ClubName, SUM(YellowCards) AS TotalYellowCards, SUM(RedCards) AS TotalRedCards
FROM Players
JOIN Clubs
ON Players.ClubID = Clubs.ClubID
GROUP BY ClubName;
...
UPDATE Players
SET SquadNumber = 8, DoB = '1999-01-06'
WHERE PlayerID = 3;
...
GO
CREATE PROCEDURE UpdateDoB (@id int, @birthday date)
AS
    UPDATE Players 
    SET DoB = @birthday 
    WHERE PlayerID = @id;

EXEC UpdateDoB @id = 2, @birthday = '1995-02-03';
...
GO
CREATE PROCEDURE DeleteClub (@name varchar(255))
AS
    DELETE FROM Clubs
    WHERE ClubName = @name;

EXEC DeleteClub @name = 'Barcelona';

CREATE TABLE UpdatePlayerNameAuditLog (
    PlayerID int,
    CreatedAt datetime,
    CreatedBy varchar(255),
    OldPlayerName varchar(30),
    NewPlayerName varchar(30)
);

GO
CREATE TRIGGER UpdatePlayerNameAudit 
ON Players AFTER UPDATE
AS
BEGIN
    INSERT INTO UpdatePlayerNameAuditLog
    SELECT i.PlayerID,
    GETDATE(),
    SUSER_SNAME(),
    d.PlayerName AS OldValue,
    i.PlayerName AS NewValue
    FROM INSERTED i
    JOIN DELETED d
...
UPDATE Players
SET PlayerName = 'Test Update'
WHERE PlayerID = 3;

SELECT *
FROM UpdatePlayerNameAuditLog;

CREATE TABLE DeletePlayerAuditLog (
    PlayerID int,
    CreatedAt datetime,
    CreatedBy varchar(255)
);
...
GO
CREATE TRIGGER DeletePlayerAudit 
ON Players AFTER DELETE
AS
BEGIN
    INSERT INTO DeletePlayerAuditLog
    SELECT PlayerID,
    GETDATE(),
    SUSER_SNAME()
    FROM DELETED
...
SELECT *
FROM DeletePlayerAuditLog;

GO
CREATE FUNCTION DisplayNameOfYouthPlayer ()
RETURNS varchar(30)
AS
BEGIN
    DECLARE @name varchar(30);
    SET @name = (SELECT TOP 1 PlayerName
    FROM Players
    ORDER BY DoB DESC);
    RETURN @name;
END;
...
EXEC @ReturnValue = DisplayNameOfYouthPlayer;
SELECT @ReturnValue AS ReturnValue;

GO
CREATE FUNCTION DisplayNumberOfMathes (@id_club int)
RETURNS int
AS
BEGIN
    DECLARE @result int;
    ...
    FROM Matches
    WHERE HomeClubID = @id_club OR AwayClubID = @id_club);
    RETURN @result;
END;

GO
DECLARE @ReturnValue int;
EXEC @ReturnValue = DisplayNumberOfMathes 1;
SELECT @ReturnValue AS ReturnValue;